...
ACCReader& ACCReader::getInstance() {
	static ACCReader instance;
	return instance;
}

int ACCReader::InitSM() {
...
void ACCReader::ReadData(void* outData) {
	ACCData* data = static_cast<ACCData*>(outData);
	ReadGraphics(&data->g);
	ReadPhysics(&data->p);
	ReadStatic(&data->s);
	memcpy(outData, data, sizeof(ACCData));
}

void ACCReader::DismissSM() {
	// Dismiss Physics
...
void ACCReader::ReadPhysics(SPageFilePhysics* outData)
{
...
void ACCReader::ReadGraphics(SPageFileGraphic* outData)
{
...
void ACCReader::ReadStatic(SPageFileStatic* outData)
{
